...
source /jffs/dualdial/program/sync_noip/dualdial_function.sh
...
[ "$w" = "0" ] && (echo_date: no.0-------------------Dualdial working!) |tee -a /tmp/syslog.log >> "${LOG_FILE}"
...
i=$(($i+1))
w=$(($w+1))
	echo_date: no.1-----count:$(echo $i|awk '{printf("%02d\n",$0)}')------dualdial dialing! >> "${LOG_FILE}"
	...
	[ "$(ps |grep pppd|grep -v grep|awk '{print $4}'|grep 'S'|wc -l)" != "0" ] && {
		n=0
		#until [ "$(route|grep "default"|wc -l)" != "0" ] ; do
	...
	if [ "$(ifconfig |grep "P-t-P"|wc -l)" -lt 2 ];then #任意IP地址,不限制所获取的IP头
		...
		echo_date: no.2-------------------No lucky,retry!!! >> "${LOG_FILE}"
		[ "$(ps |grep wanduck|grep -v grep|wc -l)" -ne 0 ] && killall wanduck >/dev/null 2>&1 &
...
#拨号阶段已结束，收尾;
echo_date: -----------------------check result!'    ' >> "${LOG_FILE}"
...
if [ "$pubip_check" = 1 ]; then
	...
	if [ "$pubweblist" != "" ]; then
		...
		do
			[ "$logdata" = "2" ] && echo_date: 'pubweb='$pubweb >> "${LOG_FILE}"
			pubippA=$(wget -t 2 -T 5 -q -O - $pubweb --bind-address=$ippA)
			pubippB=$(wget -t 2 -T 5 -q -O - $pubweb --bind-address=$ippB)
			[ "$logdata" = "2" ] && echo_date: 'pub_wan0='$pubippA//'pub_wan1='$pubippB >> "${LOG_FILE}"
			if [ "$pubippA" != "" -a "$pubippB" != "" ]; then
				break
			fi
			[ "$logdata" = "2" ] && echo_date: pubweb check done >> "${LOG_FILE}"
		done
		if [ "$ippA" != "$pubippA" -o "$ippB" != "$pubippB" ]; then
			[ "$logdata" != "0" ] && echo_date: IP地址为内网IP，将重新拨号! >> "${LOG_FILE}"
			reload_dualdial
		fi
	else
		[ "$logdata" != "0" ] && echo_date: 外网验证网址列表为空，无法判断是否为内网IP! >> "${LOG_FILE}"
	fi
fi
#负载均衡检查（过滤异常）（不适用单线拨号）
n=0
...
if [ "$lb_UH" -gt 2 -o "$nexthopnum" -lt 2 ];then
	[ "$loadbalance_check" = "0" ] && echo_date: `echo -e "\033[32;49;5m"please remind----------"\033[0m"`sys route/lb abnormal!UH SUM:$(route |grep -c 'UH')/LB SUM:$nexthopnum >> "${LOG_FILE}"
	[ "$loadbalance_check" = "1" ] && echo_date: `echo -e "\033[32;49;5m"reload dualdial--------"\033[0m"`sys route/lb abnormal!UH SUM:$(route |grep -c 'UH')/LB SUM:$nexthopnum >> "${LOG_FILE}"
...
if [ "$nexthopnum" = "2" ];then
	if [ "$(ps |grep pppd|grep -v grep|awk '{print $4}'|grep 'S'|wc -l)" -gt 2 ]; then
		[ "$loadbalance_check" = "0" ] && echo_date: `echo -e "\033[32;49;5m"please remind----------"\033[0m"`too much pppd process!UH SUM:$(route |grep -c 'UH') /LB SUM:$(ip route|grep 2>/dev/null nexthop|wc -l) /PPPD SUM:$(ps |grep pppd|grep -v grep|grep 'S'|wc -l)  >> "${LOG_FILE}"
		[ "$loadbalance_check" = "1" ] && echo_date: reload dualdial------too much active pppd process!UH SUM:$(route |grep -c 'UH') /LB SUM:$(ip route|grep 2>/dev/null nexthop|wc -l) /PPPD SUM:$(ps |grep pppd|grep -v grep|grep 'S'|wc -l) >> "${LOG_FILE}"
		[ "$loadbalance_check" = "1" ] && reload_dualdial #自定义函数
	fi
fi
...
if [ "$(ps |grep wanduck|grep -v grep|wc -l)" != "0" ];then
	if [ "$fix_lanwan" = "1" ];then
		echo wanduck re_found,reload dualdial.sh >> "${LOG_FILE}"
...
echo_date: ----Congratulation!----Gateway（WAN0/WAN1）:"$gateway0"/"$gateway1"`echo -e "\033[36;49;5m"//"\033[0m"`IP（WAN0/WAN1）:`echo -e "\033[33;49m"$ippA/$ippB"\033[0m"` >> "${LOG_FILE}"
...
if [ "$autoredial_enable" = "3" ];then
	dbus remove `dbus list __delay 2>/dev/null|grep dualdial_check|awk -F \= '{print $1}'` >/dev/null 2>&1 &
	cru d dualdial_check >/dev/null 2>&1 &
	sleep 2 && /jffs/dualdial/program/dualdial_check.sh &
fi
exit 0
